...
#
# Modified the optimizer to refer to columns of weights
class UnregularizedAdagradTrainer(Optimizer):
    ...
    def apply_gradient_update(self, gradient: Counter, batch_size: int):
        ...
        #
        # NOTE:
        # Weights are a (num_features)x(num_labels)-size matrix
        # and self.weights[i][:] is a row of weights that would
        # correspond to the indexed weights for each of the 
        # labels if the weights were flattened to a vector
        for i in gradient.keys():
            xti = self.weights[i][:]
            gti = -gradient[i] * batch_size_multiplier
            self.diag_Gt[i][:] += gti**2.0
            Htii = 1 + np.sqrt(self.diag_Gt[i][:])
            eta_over_Htii = self.eta / Htii
